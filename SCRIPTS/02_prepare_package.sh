...
# 设置LAN口IP地址为DHCP获取，关闭LAN口DHCP SERVER功能。
sed -i "s/set network\.\$1\.proto='static'/set network\.\$1\.proto='dhcp'/g" package/base-files/files/bin/config_generate
sed -i "s/set network\.\$1\.ipaddr='\$ipad'/set dhcp\.\$1\.ignore='1'/g" package/base-files/files/bin/config_generate
